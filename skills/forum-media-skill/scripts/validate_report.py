...
CHECK_NAMES = ("valid-json", "dry-run", "full-run-gate", "coverage-ratio", "recovery-required", "forum-payload")
...
def check_forum_payload(data: dict) -> bool:
    """The report must expose a structural forum block."""
    forum = data.get("forum")
    if not isinstance(forum, dict):
        return False
    counts = forum.get("entity_counts")
    if not isinstance(counts, dict) or not counts:
        return False
    if "paginated_posts_seen" not in forum:
        return False
    cov = forum.get("attachment_coverage")
    if cov == "n/a":
        return True
    try:
        return 0.0 <= float(cov) <= 1.0
...
CHECKS = {
    ...
    "forum-payload": check_forum_payload,
}
